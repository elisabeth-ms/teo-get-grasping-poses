...
bool GetGraspingPoses::configure(yarp::os::ResourceFinder &rf)
{
  ...
  else
  {
    ...
    for (unsigned int joint = 0; joint < 2; joint++)
    {
      if (strRGBDRemote == "/teo/realsense2")
      {
        qrMin.addFloat64(-60);
  ...
  rosNode = std::make_shared<rclcpp::Node>("get_grasping_poses_node");

  pointCloud_outTopic = rosNode->create_publisher<sensor_msgs::msg::PointCloud2>("/depthToPointCloud", 10);
  pointCloudLccpTopic = rosNode->create_publisher<sensor_msgs::msg::PointCloud2>("/PointCloudLccp", 10);
  pointCloudFillingObjectsTopic = rosNode->create_publisher<sensor_msgs::msg::PointCloud2>("/pointCloudFillingObjects", 10);
  graspingPoses_outTopic = rosNode->create_publisher<visualization_msgs::msg::MarkerArray>("/graspingPoses", 10);
  bbox3d_outTopic = rosNode->create_publisher<visualization_msgs::msg::MarkerArray>("/bbox3d", 10);
...
bool GetGraspingPoses::updateSuperquadricsObjects()
{
  ...
  if (m_update_data)
  {
    ...
    if (!iRGBDSensor->getImages(rgbImage, depthFrame, &colorStamp, &depthStamp))
    {
      yError() << "getImages failed";
      return false;
    }
    ...
    // intrinsic_ok = iRGBDSensor->getRgbIntrinsicParam(propIntrinsic);

    // yarp::sig::IntrinsicParams intrinsics(propIntrinsic);
    yarp::sig::ImageOf<yarp::sig::PixelRgb> colorImagePixelRGB;
    colorImagePixelRGB.setExternal(rgbImage.getRawImage(), rgbImage.width(), rgbImage.height());
    // create point cloud in yarp format
    // yarp::sig::PointCloud<yarp::sig::DataXYZRGBA> pcFiltered = yarp::sig::utils::depthRgbToPC<yarp::sig::DataXYZRGBA, yarp::sig::PixelRgb>(depthFrame,
    // colorImagePixelRGB,
    // intrinsics,
    ...
    if (transformPointCloud(pcFiltered, transformed_cloud, true))
    {
      ...
      if (table_removed && without_horizontal_surface_cloud->points.size() > 100)
      {
        ...
        for (int i = 0; i < without_horizontal_surface_cloud->points.size(); ++i)
        {

          pcl::PointXYZ tmp_point_projected;
          tmp_point_projected.x = without_horizontal_surface_cloud->points[i].x;
          tmp_point_projected.y = without_horizontal_surface_cloud->points[i].y;
          tmp_point_projected.z = DEFAULT_TABLE_HEIGHT_WRT_TRUNK;

          cloud_without_table_projected->points.push_back(tmp_point_projected);
        }

        yInfo() << "We have " << cloud_without_table_projected->points.size() << " points in the projected cloud.";
        ...
        for (const auto &cluster : cluster_indices)
        {
          ...
          {
            pcl::PointXYZL point;
            point.x = without_horizontal_surface_cloud->points[idx].x;
            point.y = without_horizontal_surface_cloud->points[idx].y;
            point.z = without_horizontal_surface_cloud->points[idx].z;
            point.label = j;
        ...
        if (lccp_labeled_cloud->points.size() != 0)
        {
          ...
          {
            yInfo() << "idx: " << idx << " label: " << detected_objects[idx].label;

            // point_cloud.readFromFile("example-bottle");
            SuperqModel::PointCloud point_cloud = PclPointCloudToSuperqPointCloud(detected_objects[idx].object_cloud);
            ...
            *cloudSuperquadric += *auxCloudSuperquadric;
            ...
            superq.setSuperqParams(superqs[0].getSuperqParams());
            object_superquadric.superqs.push_back(superq);
...
bool GetGraspingPoses::getTransformMatrix(const bool &from_camera_to_trunk, Eigen::Matrix4f &transform)
{
  ...
  {
    yError() << "iTunkEncoders getEncoders() failed";
    return false;
  }
  ...
  yInfo() << "currentQ: " << currentQ[0] << " " << currentQ[1] << " " << currentQ[2] << " " << currentQ[3];
  yInfo() << "currentX: " << currentX[0] << " " << currentX[1] << " " << currentX[2] << " " << currentX[3] << " " << currentX[4] << " " << currentX[5] << " " << currentX[6];
  ...
  if (robot == "/teo")
  {
    ...
    else if (strRGBDRemote == "/teo/realsense2")
    {
      std::cout << "/realsense2" << std::endl;
      KDL::Frame frame_camera_head;
      // frame_camera_head = frame_camera_head*KDL::Frame(KDL::Vector(0.0,0.0,0.0));
      frame_camera_head = frame_camera_head * KDL::Frame(KDL::Vector(0.0, 0.0175+0.015, 0.0));
      frame = frame_head_trunk * frame_camera_head;
  ...
  yInfo()<<"From camera to trunk: "<<from_camera_to_trunk;
  yInfo()<<"Frame Pos: "<<frame.p.x()<<" "<<frame.p.y()<<" "<<frame.p.z();
...
/************************************************************************/
void GetGraspingPoses::rosComputeAndSendPc(const yarp::sig::PointCloud<yarp::sig::DataXYZRGBA> &yarpCloud, std::string frame_id, rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr PointCloudTopic)
{
  sensor_msgs::msg::PointCloud2 pc2Ros;

  pc2Ros.header.frame_id = frame_id;
  pc2Ros.header.stamp = rclcpp::Clock().now();
  ...
  PointCloudTopic->publish(pc2Ros);
...
SuperqModel::PointCloud GetGraspingPoses::PclPointCloudToSuperqPointCloud(const pcl::PointCloud<pcl::PointXYZRGBA> &object_cloud)
{
  std::vector<Eigen::Vector3d> acquired_points;
  std::vector<std::vector<unsigned char>> acquired_colors;

  std::deque<Eigen::Vector3d> all_points;
  for (size_t idx = 0; idx < object_cloud.size(); idx++)
  {

    Eigen::Vector3d p(3);

    p(0) = object_cloud[idx].x;
    p(1) = object_cloud[idx].y;
    p(2) = object_cloud[idx].z;

    std::vector<unsigned char> c(3); // Color vector
    c[0] = (unsigned char)object_cloud[idx].r;
    c[1] = (unsigned char)object_cloud[idx].g;
    c[2] = (unsigned char)object_cloud[idx].b;

    all_points.push_back(p);
    acquired_colors.push_back(c);
  }

  SuperqModel::PointCloud point_cloud1;

  point_cloud1.setPoints(all_points);
  point_cloud1.setColors(acquired_colors);
  return point_cloud1;

  // if (!point_cloud.readFromFile("example-bottle"))
  //   return false;
  // return true;
}

std::vector<SuperqModel::Superquadric> GetGraspingPoses::GetSuperquadricFromPointCloud(SuperqModel::PointCloud &point_cloud)
{
  yInfo() << "n_points: " << point_cloud.n_points;
  ...
  yInfo() << "Superquadric computed!";

  // yInfo() << "Number of superquadrics: " << superqs.size();
  return superqs;
}
    ...
    for (float xaxes = -1.0; xaxes <= 1.0; xaxes += 2)
    {
      ...
      for (float y = -params[1] / 2.0; y <= params[1] / 2.0; y += step)
      {
        for (float yaxes = -1.0; yaxes <= -1.0; yaxes += 2)
        {
          ...
          printf("angle: %f\n", angle * 180.0 / M_PI);
          if (abs(angle) > 0 * M_PI / 180.0)
          {
            printf("We add a grasping pose\n");
...
bool GetGraspingPoses::createBoundingBox2DFromSuperquadric(const std::vector<SuperqModel::Superquadric> &superqs, std::array<int, 4> &bbox)
{
  ...
  {
    yInfo("Publish...\n");
    // bbox3d_outTopic->publish(markerArray);
  }
...
void GetGraspingPoses::rosSendGraspingPoses(const std::string &frame_id, const std::vector<std::vector<double>> &graspingPoses)
{
  ...
  {
    yInfo("Publish...\n");
    graspingPoses_outTopic->publish(markerArray);
  }
